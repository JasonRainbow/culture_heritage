    ...
    <el-row :gutter="40">
      <el-col :span="10" :xs="24" :sm="24" :lg="10">
        <el-card class="box-card grid-content" shadow="always">
          ...
          <el-card v-for="item in list_data" :key="item.cultureId"
...
import {searchHotHeritagesByPage} from "@/api/search_culture_heritage";
...
export default {
  ...
  data() {
    return {
      ...
      list_data: [
        {
          name: '湘绣',
          introduction: '湘绣是中国四大名绣之一，是以湖南长沙为中心的带有鲜明湘楚文化特色的湖南刺绣产品的总称，它起源于湖南的民间刺绣，吸取了苏绣和粤绣的优点而发展起来，已经有2000多年历史。',
        ...
        {
          name: '土家族织锦',
          introduction: '土家族织锦的工艺复杂，在织造上，使用古老的纯木质腰式斜织机织造，需经过纺捻线、染色、倒线、牵线、装筘、滚线、捡综、翻篙、捡花、捆杆上机、织布、挑织等12道工序制作而成。',
        ...
        {
          name: '蓝印花布印染技艺',
          introduction: '蓝印花布印染技艺分纯蓝花布印染和彩蓝花布印染两大类：纯蓝花布布面素净，没有花纹图案，一般用以制作衣服和头帕；彩蓝花布有复杂的花纹图案，一般用以制作被面、垫单、帐檐、围裙、围腰、门帘、桌布、围布等。',
  ...
  methods: {
    handleCurrentChange(val) {
      ...
      this.getHotHeritages()
    },
    handleSizeChange() {
    },
    getHotHeritages() {
      searchHotHeritagesByPage(this.searchParams).then((res)=>{
        if (res.code === "0") {
